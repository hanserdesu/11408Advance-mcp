...
| `print_strict` | 必须打印，打不出来就报错、不退化 | 靠「必须落笔」逼自己的备考纪律党 |
...
| 通道 | 依赖 | 说明 |
|---|---|---|
| `print` | Windows：装有 SumatraPDF；macOS/Linux：有 `lp`/`lpr`；外加一个 Chromium 系浏览器 | 先用无头浏览器把 HTML 渲染成 PDF，再送去实体打印 |
