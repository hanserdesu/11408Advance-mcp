...
- **2026-09-20**：
  - **11408 内容包全量刷新与真题分值档逐章复核**：全库 58 章 / 2455 原子 / 696 组合分值档（S/A/B）全部完成对照真题档案原文逐章复核，全部合规且零虚高降档；内容包清洗导出管线重跑（个人实测记录、会话痕迹与本地路径零导出，全部原子状态以 ⬜ 起步分发）。
  - **学习算法收敛与先验时近加权**：算法参数化拟合收敛到活跃集（在学章节与活跃复习项约 50-60 个原子），全库其余原子走规则层分值档；先验难度计算引入 14 天半衰期时近加权，避免早期孤立失误造成永久惩罚；先预测后作答完成整卷实跑闭环。
  - **出卷交付与运行冷启动性能提升**：出卷管线在无符号计算时跳过 sympy 导入，渲染冷启动耗时从 1.25s 压降至 0.13s（近 10 倍提速）；打印就绪检测消除固定 1s 盲等，改细粒度可读就绪轮询（打印耗时再减约 25%）；测试套件 63 项全绿。
- **2026-09-17**：打印通道实测修复（虚拟打印机不再被自动选中、`auto` 策略真实出纸、页数超估算时重排）；打印工具探测抽成 `detect_print_tool`，CI 在 Linux 上可跑。
- **2026-09-16**：内容包与兼容预留——`11408` 知识库随仓库分发，新增 `pack` 子命令与 `schema_version` 守卫。
