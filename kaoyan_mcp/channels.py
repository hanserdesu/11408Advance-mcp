...
def probe(ws=None):
    ...
    default_printer = policy.get("printer") or (printers[0] if printers else "")
    print_tool = ""
    if IS_WINDOWS and sumatra:
        print_tool = "sumatra"
    elif not IS_WINDOWS and lp:
        print_tool = "lp"
    channels = {
        "pdf": bool(browser),
        "html": True,
    ...
    if not default_printer:
        reasons["print"] = "未检测到打印机（可用 config.deliver.printer 指定）"
    elif not print_tool:
    ...
    return dict(os="windows" if IS_WINDOWS else ("macos" if IS_MAC else "linux"),
                ...
                printers=printers, printer=default_printer, print_tool=print_tool,
...
def deliver(ws, built, spec=None, policy=None):
    ...
    if want in ("print_strict", "print"):
        if info["channels"]["print"]:
            if _print():
                result.update(ok=True, channel="print")
                return result
            if want == "print_strict":
                result["warnings"].append("打印失败，严格模式下不退化：请处理打印机后重试")
                return result
        else:
    ...
    if want in ("print_strict", "print", "auto", "pdf") and info["channels"]["pdf"] and _pdf():
        ...
        if want in ("print", "print_strict"):
            result["warnings"].append("未能打印，已退化为 PDF（可自行打开打印）")
        return result
